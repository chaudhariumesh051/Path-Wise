...
const LessonView: React.FC = () => {
  ...
  const [playgroundValues, setPlaygroundValues] = useState<{[key: string]: any}>({});
  ...
  const handleMarkComplete = async () => {
    try {
      ...
      setIsCompleted(true);
      showSuccess('Lesson Completed', 'Great job! You can now move to the next lesson.');
      ...
      if (progressData) {
        ...
        const newAverageAccuracy = (oldAverageAccuracy * oldCompletedLessons + 100) / newCompletedLessons;
      ...
      showError('Save Failed', 'Failed to mark lesson as complete. Please try again.');
    }
  };
  ...
  const renderInteractiveElement = (element: any, index: number) => {
    switch (element.type) {
      case 'playground':
        return (
          <div key={index} className="bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-lg p-4">
            <h4 className="font-medium text-light-text-primary dark:text-dark-text-primary mb-2">{element.title}</h4>
            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mb-4">{element.description}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-2">Input</label>
                <textarea
                  value={playgroundValues[`playground_${index}`] || ''}
                  onChange={(e) => setPlaygroundValues(prev => ({...prev, [`playground_${index}`]: e.target.value}))}
              ...
              <div>
                <label className="block text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-2">Output</label>
                <div className="w-full p-3 border border-light-border dark:border-dark-border rounded-lg bg-light-bg dark:bg-dark-bg text-light-text-primary dark:text-dark-text-primary min-h-[100px]">
                  {playgroundValues[`playground_${index}`] || 'Output will appear here...'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div key={index} className="bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-lg p-4">
            ...
            <div className="bg-white dark:bg-dark-card border border-light-border dark:border-dark-border rounded-lg p-4">
              <div className="text-center">
                ...
                <p className="text-light-text-primary dark:text-dark-text-primary">Interactive demo placeholder</p>
              </div>
            </div>
          </div>
        );
      ...
      default:
        return (
          <div key={index} className="bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-lg p-4">
            <h4 className="font-medium text-light-text-primary dark:text-dark-text-primary mb-2">{element.title}</h4>
            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">{element.description}</p>
          </div>
        );
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-light-text-secondary dark:text-dark-text-secondary">Loading lesson...</p>
        </div>
      </div>
    );
  }
  ...
  if (!lesson || !lessonContent) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-2 text-sm font-medium text-light-text-primary dark:text-dark-text-primary">Lesson not found</h3>
        <button 
          onClick={fetchLesson}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-dark-card rounded-xl shadow-sm p-6 border border-light-border dark:border-dark-border">
        ...
        <h1 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary mb-2">{lessonContent.title}</h1>
        ...
        <>
          ...
          <div className="bg-white dark:bg-dark-card rounded-xl shadow-sm p-6 border border-light-border dark:border-dark-border">
            <div className="prose max-w-none">
              ...
              {/* Key Concepts */}
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary mb-3">Key Concepts</h3>
                <ul className="space-y-2">
                  {lessonContent.keyConcepts.map((concept, index) => (
                    <li key={index} className="flex items-start space-x-2">
                      ...
                      <span className="text-light-text-primary dark:text-dark-text-primary">{concept}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {lessonContent.interactiveElements && lessonContent.interactiveElements.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary mb-3">Interactive Learning</h3>
                  <div className="space-y-4">
                    {lessonContent.interactiveElements.map((element, index) => renderInteractiveElement(element, index))}
                  </div>
                </div>
              )}
              ...
              {lessonContent.exampleCode.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary mb-3">Examples</h3>
                  <div className="space-y-4">
                    {lessonContent.exampleCode.map((example, index) => (
                      <div key={index} className="border border-light-border dark:border-dark-border rounded-lg overflow-hidden">
                        <div className="bg-light-bg dark:bg-dark-bg px-4 py-2 border-b border-light-border dark:border-dark-border">
                          <p className="text-sm font-medium text-light-text-primary dark:text-dark-text-primary">{example.description}</p>
                        </div>
                        <div className="p-4">
                          ...
                          {example.output && (
                            <div className="mt-3">
                              <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mb-2">Output:</p>
                              <div className="bg-light-bg dark:bg-dark-bg p-3 rounded-lg">
                                <code className="text-sm text-light-text-primary dark:text-dark-text-primary">{example.output}</code>
                              </div>
                            </div>
                          )}
                        </div>
          ...
          {!testCompleted ? (
            <>
              ...
              <div className="space-y-4">
                ...
                {lessonContent.assessmentQuestions[currentQuestion].type === 'multiple-choice' ? (
            ...
            <div className="text-center py-8">
              <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${
                score >= 70 ? 'bg-light-success/10 dark:bg-dark-accent/10' : 'bg-light-error/10 dark:bg-dark-error/10'
              }`}>
                {score >= 70 ? (
                  <Award className={`w-8 h-8 text-light-success dark:text-dark-accent`} />
                ) : (
                  <RotateCcw className={`w-8 h-8 text-light-error dark:text-dark-error`} />
                )}
              </div>
              <h3 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary mb-2">Test Complete!</h3>
              <p className="text-lg text-light-text-secondary dark:text-dark-text-secondary mb-4">Your score: {score}%</p>
              ...
              {showAnswers && (
                <div className="text-left space-y-4 border-t border-light-border dark:border-dark-border pt-6">
                  <h4 className="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary">Answer Review</h4>
                  {lessonContent.assessmentQuestions.map((question, index) => {
                    ...
                    return (
                      <div key={index} className="bg-light-bg dark:bg-dark-bg rounded-lg p-4">
                        <div className="flex items-start space-x-2 mb-2">
                          ...
                          <div className="flex-1">
                            <p className="font-medium text-light-text-primary dark:text-dark-text-primary">{question.question}</p>
                            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-1">
                              Your answer: <span className={isCorrect ? 'text-light-success dark:text-dark-accent' : 'text-light-error dark:text-dark-error'}>{userAnswer || 'No answer'}</span>
                            </p>
                            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">
                              Correct answer: <span className="text-light-success dark:text-dark-accent">{question.answer}</span>
                            </p>
                            {question.explanation && (
                              <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-2 italic">{question.explanation}</p>
                            )}
                          </div>
                        </div>
                      </div>
